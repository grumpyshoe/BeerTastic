...
class SharedPreferenceServiceImpl @Inject constructor(
    ...
    override fun checkIfFavorite(beerId: Int): Boolean {
        return encryptedSharedPrefs.getString(SharedPreferenceService.KEY_FAVORITES, null).let { favorites ->
    ...
    override fun setIsBeerFavorite(beerId: Int, isFavorite: Boolean) {
        ...
        val updatedList = favoriteList.apply {
            if (isFavorite) {
                add(beerId)
            } else {
                remove(beerId)
        ...
        encryptedSharedPrefs.edit(true) {
            putString(SharedPreferenceService.KEY_FAVORITES, updatedList.joinToString(separator = ","))
        }
        _favorites.tryEmit(updatedList)
    }

    private fun getFavorites(): List<Int> {
        return encryptedSharedPrefs.getString(SharedPreferenceService.KEY_FAVORITES, null)?.split(",")
            ?.mapNotNull {
                try {
                    it.toInt()
                } catch (e: Exception) {
